    ...
    testutil.befortest(serial, adb, log)
      ...
      .then(function () {
        ...
        if (typeof options.pkg == 'string') {
          return testutil.uninstallAPK(adb, serial, options.pkg, log)
        } else {
          options.pkg.forEach(function (pkg) {
            return testutil.uninstallAPK(adb, serial, pkg, log)
          })
        }
      })
      .then(function () {
...
testutil.wakeUpPhone = function (serial, adb, log) {
  ...
  return new Promise(function(resolve, reject){
    ...
    wake(serial, adb, log, function(err){
      if(err){
        reject(err)
      }
      resolve('')
  ...
  function wake(serial, adb, log, cb){
    ...
    adb.shell(serial, 'pm clear com.example.unlock')
      ...
      .then(function(){
        var backArr = ['68U5T17A08019967', '36LBB17922519073']
        if(backArr.indexOf(serial) > 0){
          return adb.shell(serial, 'dumpsys activity activities')
            .then(adbkit.util.readAll)
            .then(function(output){
              if(/mFocusedActivity.*PluginLoaderActivityForShopProcess/i.test(output.toString())){
                return adb.shell(serial, 'input keyevent KEYCODE_BACK')
              }
            })
        }
      })
        ...
        adb.shell(serial, 'dumpsys window policy')
          ...
          .then(function(output){
            ...
            if(mShowingLockscreen == 'false' && isStatusBarKeyguard == 'false' || wakeCount > 3){
              ...
              var swipeUpArr = ['da4db2db', 'fe392d3e', '8cedb476']
              if(swipeUpArr.indexOf(serial) > 0){
                adb.shell(serial, 'input swipe 500 1000 500 300')
              }
      ...
      .catch(function(err){
        log.debug('wake up phone error:', err)
        cb(err)
      })
  }
}
...
testutil.killAndroidProceBypkg = function (adb, serial, pkg, log) {
  ...
  return adb.shell(serial, 'input keyevent KEYCODE_HOME')
    ...
    .then(function(){
      ...
      return adb.getProperties(serial)
    })
    .then(function(properties){
      return properties['ro.build.version.sdk']
    })
    .then(function(sdk){
      log.debug('sdk:'+sdk)
      var pscmd = 'ps'
...
testutil.monitorApp = function (adb, serial, logpath, log) {
  var logd = ''
  adb.shell(serial, 'am monitor')
    .then(function(stream){
      stream.on('data', function(data){
        ...
        logd = logd + d
        if(/(k)ill:/.test(d)){
          stream.write(stream.write('k\n'))
        }
        log.debug('monitor app data:' + d)
      })

      stream.on('end', function(){
        log.debug('monitor app end.')
        fs.appendFileSync(logpath, logd)
        logd = ''
      })
    })
}
...
testutil.uninstallAPK = function(adb, serial, pkg, log){
  return testutil.killAndroidProceBypkg(adb, serial, pkg, log)
    .then(function(){
      return adb.shell(serial, 'pm clear ' + pkg)
    })
    .then(function(){
      return adb.uninstall(serial, pkg)
    })
