      ...
      if (path.extname(path_) == '.py') {
        ...
        exec(command, function (err, stdout, stderr) {
          ...
          if (data.length) {
            ...
            redisutil.onFileAdded(data).then(function (_data) {
              log.info("redis file add completed!")
            })
  ...
  function fileChangeListener(path_) {
    if (path.extname(path_) == '.py') { 
	    ...
	    if(data.length) {
          ...
          redisutil.onFileChanged(path_, data).then(function (data_) {
            log.info("redis file change completed!")
          })
...
// 删除文件时，需要把文件里所有的用例删掉
  ...
  function fileRemovedListener(path_) {
    if (path.extname(path_) == '.py') {
      ...
      redisutil.onFileRemoved(path_).then(function() {
        log.info("redis file removed completed!")
      })
  ...
  function directoryRemovedListener(path_) {
    ...
    redisutil.onDirectoryRemoved(path_).then(function() {
      log.info("redis directory removed completed!")
    })
...
module.exports.extname = function(file) {
