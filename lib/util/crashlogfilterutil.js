...
crashlogfilterutil.logcatentrieslogfilter = function (entries, pidArr, pkg, callback) {
    ...
    for(var i = 0; i<entries.length; i++){
                ...
                if(messages.length>1){
                    for(var j = 1; j<messages.length; j++){
                        if(runtimeexecptionError.test(messages[j])){
                            result = {errorType : 'crash', reason : messages[j]}
        ...
        if(anrError.test(entry)){
            result = {errorType : 'anr', reason : entry.message.split('\n')[0]}
            return callback(result)
        }
        ...
        if(fatalName.test(entry.message)){
            ...
            for(var k = 0; k<fatalArray.length; k++){
                if(nameRegExp.test(fatalArray[k])){
                    result = {errorType : 'crash', reason : fatalArray[k]}
                    return callback(result)
                }
            }
        }
        ...
        if(nostatediedError.test(entry.message)){
            result = {errorType : 'crash', reason : entry.message}
            return callback(result)
        }
...
crashlogfilterutil.monkeylogfilter = function (outputStr, callback) {
    ...
    var failTalkRegExp = new RegExp('Failed talking with activity manager', 'i')
            ...
            if(nativecrashRegExp.test(outputStrArr[i])){//如果是native crash时 Long Msg对应的可能不是真正的crash原因
                for(var j = i; j<outputStrArr.length; j++){
                    if(nativeMsgRegExp.test(outputStrArr[j])){
                        result = {errorType : 'crash', reason : outputStrArr[j+1].split('//')[1].trim()}
                        log.debug('result : ',result)
                        return callback(result)
                    }
                }
            }
            result = {errorType : 'crash', reason : outputStrArr[i].replace(longMsgRegExp, '')}
            log.debug('result : ',result)
            break
        }else if(anrRegExp.test(outputStrArr[i])){
            result = {errorType : 'anr', reason : outputStrArr[i]}
            log.debug('result : ',result)
            break
        }else if(exceptionRegExp.test(outputStrArr[i])){
            result = {errorType : 'anr', reason : outputStrArr[i+1]}
            break
        }else if(anrtraceRegExp.test(outputStrArr[i])){
            result = {errorType : 'anr', reason : outputStrArr[i]}
            break
        }else if(monkeyAbortedRegExp.test(outputStrArr[i])){
            result = {errotType : 'crash', reason : outputStrArr[i]}
        }else if(failTalkRegExp.test(outputStrArr[i])){
            result = {errotType : 'other', reason : 'Failed talking with activity manager'}
            break
        }
    }
...
crashlogfilterutil.logcatfilter = function (outputStr, callback) {
    var signalError = new RegExp('Process [^]* exited due to signal')
    var result
    var outputStrArr = outputStr.split('\n')

    for (var i = 0; i < outputStrArr.length; i++) {
        if (signalError.test(outputStrArr[i])) {
            result = {errorType: 'crash', reason: outputStrArr[i].split('/')[1].trim()}
            log.debug('result : ', result)
            break
        }
        return callback(result)
