  ...
  function run(host){
    ...
    function runTask(host, taskInfo) {
      ...
      testutil.wakeUpPhone(serial, adb, log)
          ...
          adb.install(serial, apkPath)
            ...
            .catch(function (err) {
              ...
              if(!err.code){
                ...
                rpldata.result = {errorType: 'install failed', reason: err.toString()}
                return error = err
              }
              ...
              switch (err.code) {
                  ...
                  return adb.uninstall(serial, pkg)
                    ...
                    .then(function () {
                      return adb.install(serial, apkPath)
                        .timeout(60000 * 5)
                        .catch(function (er) {
                          log.debug('reInstall apk failed 11111. err:', er)
  
                          rpldata.status = 'fail'
                          rpldata.statusCode = 1
                          if (er.code) {
                            rpldata.result = {errorType: 'install failed', reason: er.code}
                          } else {
                            rpldata.result = {errorType: 'install failed', reason: er.toString()}
                          }
                    ...
                    .catch(function(er){
                      log.debug('option fail. error:' + er)
                      error = err
                      rpldata.status = 'fail'
                      rpldata.statusCode = 1
                      rpldata.result = {errorType: 'install failed', reason: err.code}
                    })
                  break
                case 'INSTALL_CANCELED_BY_USER':
                  return testutil.wakeUpPhone(serial, adb, log)
                    ...
                    .catch(function(er){
                      log.debug('INSTALL_CANCELED_BY_USER option fail. error:' + er)
                      error = err
                      rpldata.status = 'fail'
                      rpldata.statusCode = 1
                      rpldata.result = {errorType: 'install failed', reason: err.code}
                ...
                case 'INSTALL_FAILED_ABORTED':
                  return adb.shell(serial, 'pm list package')
                    ...
                    .catch(function(er){
                      log.debug('INSTALL_FAILED_ABORTED option fail. error:' + er)
                      error = err
                      rpldata.status = 'fail'
                      rpldata.statusCode = 1
                      rpldata.result = {errorType: 'install failed', reason: err.code}
                ...
                case 'INSTALL_FAILED_CONFLICTING_PROVIDER':
                  return adb.shell(serial, 'pm list package')
                    ...
                    .catch(function(er){
                      log.debug('INSTALL_FAILED_CONFLICTING_PROVIDER option fail. error:' + er)
                      error = err
                      rpldata.status = 'fail'
                      rpldata.statusCode = 1
                      rpldata.result = {errorType: 'install failed', reason: err.code}
        ...
        .catch(function(err){
          log.debug('wake up phone timeout. err:', err)
          rpldata.status = 'fail'
          rpldata.statusCode = 1
          rpldata.result = {errorType: 'install failed', reason: 'wake up phone error'}
          cb(err, rpldata)
          // installTaskFinish = true 
        })
        .finally(function(){
