    ...
    initSubServer(hostList, function () {
      ...
      setInterval(function () {
        if(!isPlanRunning){
          ...
          plan()
            .then(function(){
              if(unfinishTasks && unfinishTasks.length > 0){
                for(var i=0; i<unfinishTasks.length; i++){
                  if(new Date().getTime()-parseInt(unfinishTasks[i].taskid.split('_')[0]) > 24*60*60*1000){
                    var taskserials = unfinishTasks[i].serials
                    for(var j=0; j<taskserials.length; j++){
                      var rpldata = {result:{}}
                      rpldata.serial = taskserials[j] 
                      rpldata.status = 'fail'
                      rpldata.statusCode = 6
                      rpldata.result = {errorType: 'task timeout', reason: 'task timeout'}
                      dbapi.saveTaskStatus(unfinishTasks[i].taskid, rpldata)
                      dbapi.unsetDeviceOwner(taskserials[j])
                    }

                    dbapi.updateTaskReport(unfinishTasks[i].taskid, {allstatus:'finish'})
                    dbapi.removedevicetask(unfinishTasks[i].taskid)
                  }
                }
              }
            })
                ...
                return Promise.promisify(cursor.toArray, cursor)()
                  .then(function (tasks) {
                    ...
                    for(var i = 0; i < tasks.length;){
                      if(tasks[i].serials.length == 0){
                        var taskid1 = tasks[i].taskid 
                        tasks.splice(i, 1)
                        
                        dbapi.removedevicetask(taskid1)
                          .then(function () {
                            return dbapi.updateTaskReport(taskid1,{allstatus:'finish'})
                          })
            ...
            testUtil.createadb({host:data.host})
              .then(function (client) {
                child.on('close', function(){
                  if(mLog && mLog.debug){
                    mLog.debug('test process finish')
                  }
