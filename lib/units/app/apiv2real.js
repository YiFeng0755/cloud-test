...
router.get('/loadProjectInfo', function (req, res) {
  ...
  var options = {
    ...
    port: 80,
    path: '/landlord/reqm_new_dc/web/index.php?r=interface%2Findex',
    method: 'GET'
  }
  httputil.get(options).then(function (data) {
    ...
    if (json_data.result && json_data.result.length != 0) {
      // 这里加个判断，写php的傻逼会在原本返回的正常结构上多加一层
      //if ('projects' in json_data.result[0])
...
router.post('/monkeytest', function (req, res) {
  log.info('monkeytest post body', req.body)
  if (!req.body.Project) {
    return res.json(540, {
      success: false
      , result: '请选择项目名'
      ...
      return Promise.promisify(cursor.toArray, cursor)()
        .then(function (list) {
          ...
          if (list.length > 0) {
            ...
            res.json({
              success: true,
              version: datautil.unique(version).map(function (element) {
                return { data: element, value: false }
              }),
              dpis: datautil.unique(dpis).map(function (element) {
                return { data: element, value: false }
              })
            })
          }
        ...
        .then(function (list) {
          ...
          if (list.length > 0) {
            ...
            version.forEach(function (ver) {
              Lversion.push({ data: ver, value: false })
            })
            width.forEach(function (wid) {
              Lwidth.push({ data: wid, value: false })
            })
            height.forEach(function (hgt) {
              Lheight.push({ data: hgt, value: false })
            })
            dpis.forEach(function (dpi) {
              Ldpis.push({ data: dpi, value: false })
            })
            res.json({
              success: true
              , version: Lversion
...
/*
 2017/3/21 拆分为一级模块 二级模块后增加接口

 */
router.get('/loadModule/:project', function (req, res) {
  dbapi.loadModule(req.params.project).then(function (cursor) {
    return Promise.promisify(cursor.toArray, cursor)()
  }).then(function (cases) {
    var key = {}
    cases.forEach(function (element) {
      element.module ? (key.hasOwnProperty(element.module) ? '' : key[element.module] = '') : ''
      element.sub_module ? (key.hasOwnProperty(element.sub_module) ? '' : key[element.sub_module] = '') : ''
    })
    res.json({
      success: true,
      result: Object.keys(key).sort()
    })
  })
})
...
/*
 V2接口，根据项目获取对应的模块和用例
 modified: 2017/3/21 修改一级模块和二级模块统一
 */
...
router.post('/filterUI', function (req, res) {
  ...
  dbapi.filterUI(taskId).then(function (obj) {
    ...
    obj.devices.forEach(function (device) {
      ...
      var failCount = 0
      var waitCount = 0
      device.cases.forEach(function (c) {
        c.result == 'True' ? ++successCount : (c.result == 'False' ? ++failCount : ++waitCount)
      })
      device.order = order
      device.sucCount = successCount
      device.failCount = failCount
      device.waitCount = waitCount
      device.sucRate = Math.round(successCount / (successCount + failCount + waitCount) * 100) + '%'
      delete device.cases
    })
    res.json({
      success: true,
...
router.post('/loadUIDetail', function (req, res) {
  ...
  dbapi.loadUITest(taskId, serial, modules, status).then(function (cursor) {
    ...
    list.forEach(function (detail) {
      // _.merge(detail, {error: detail.error.replace(/ /g, '&nbsp;').replace(/\n/g, '<br />')})
...
router.get('/img/:taskId/:serial/:caseId/:num', function (req, res) {
  var taskId = req.params.taskId
  var serial = req.params.serial
  var caseId = req.params.caseId
  var num = req.params.num

  var imgs = fileutil.list('/tmp/img')
  if (num > imgs.length) {
    return res.json({
      statuscode: 101,
      result: 'Image too long!'
  ...
  res.set('Content-Type', 'image/png')
  res.sendFile(fileutil.join('/tmp/img/', imgs[num - 1].fileName))
})
...
router.post('/rerun', function (req, res) {
  ...
  dbapi.loadFailTestCaseIds(taskId, serial).then(function (taskInfo) {
    ...
    if (target_ids.length) {
      ...
      if (!req.body.memo || !req.body.memo.startsWith('【用例重试】')) {
  ...
  dbapi.getSerialImgs(taskid, serial).then(function (cursor) {
    ...
    res.json({
      ...
      imgs: imgPath.map(function (img) {
        return 'image/' + taskid + img.split(taskid).pop()
      })
    })
  })
})
...
router.get('/asset/add', function (req, res) {
...
router.post('/asset/add', function (req, res) {

  // 资产编号 
  var assetNum = req.body.assetNum
  // 手机名称
...
router.get('/assert/export', function (req, res) {
...
router.post('/task/process', function (req, res) {
  ...
  dbapi.getProcess(taskids).then(function (cases) {
    ...
    cases.forEach(function (caseinfo) {
      ...
      result[taskid] = Math.round(completedCaseCount / (serialsCount * allCaseCount)) + '%'
    })
    res.json({
      success: true,
      result: result
  ...
  dbapi.getCaseProcess(taskid).then(function (obj) {
    ...
    obj.serials.forEach(function (serial) {
      ...
      if (fileutil.exists(path)) {
        ...
        result[serial] = Math.round(completedCase.length / obj.cases.length * 100) + '%'
      }
      else {
        result[serial] = '0%'
      }
...
router.post('/pack/apk', function (req, res) {
  ...
  new Promise(function (resolve, reject) {
    ...
    //console.log(_.compact(_.uniq(Array.prototype.concat.apply([], result.cases))))
    //console.log(_.compact(_.uniq(Array.prototype.concat.apply([], result.serials))))
    //console.log(_.compact(_.uniq(Array.prototype.concat.apply([], result.version))))
...
router.get('/:taskid/:serial/report', function (req, res) {
  ...
  dbapi.loadActivityDetail(taskid).then(function (task) {
    ...
    if (task) {
      ...
      archive.pipe(res)
      archive.directory(fileutil.join(logPath, '/'), target.markName)
      archive.finalize()
    }
    else {
      return res.json({
