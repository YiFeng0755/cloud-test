/**
 * Created by YoungLiu on 2018/4/20.
 */
...
exec('python /var/stf/vendor/scancase/password.py',function(error,stdout,stderr){
    log.info(stdout)
    output = JSON.parse(stdout)
    username = output.SVN_USER_NAME
    password = output.SVN_PASSWD
    log.info(username)
    log.info(password)
    SvnCmd = 'svn update --no-auth-cache --username '+username+' --password '+password
    log.info(SvnCmd)
})
                            ...
                            task.pycharmProject.forEach(function (pyproject) {
                                ...
                                if (fileutil.exists(reportPath)) {
                                    ...
                                    _.remove(cs, function (n) {
                                        return n.fileName == 'screenshot' || n.fileName == 'TestReport.xml' || n.fileName == 'TestReport.xsl' || n.fileName == 'TestResult.xsl' || fileutil.extname(n.fileName) != '.xml'
                                    })
...
router.get('/loadProjectName', function (req, res) {
    ...
    httputil.get(options).then(function (data) {
        ...
        if (json_data.result && json_data.result.length != 0) {
            ...
            _.forEach(json_data.result, function (n) {
                _.forEach(n.projects, function (p) {
                    projectNames.push({ label: p.projectName, value: p.projectId })
                })
            })
            res.json({ success: true, projectNames: projectNames })
        }
        else {
            var projectNames = []
...
router.get('/loadTestSuite', function (req, res) {
    ...
    type = type ? (type == 'p' ? '性能项目' : (type == 't' ? '遍历测试' : null)) : null
    condition = type ? condition + type + ':' : condition
    condition = project ? condition + project + ':' : condition
    condition = version ? condition + version + ':' : condition
    condition = suite ? condition + suite : condition + '*'

    if (global.useredis) {
        exec(SvnCmd, { cwd: '/var/stf/testproject2' }, function (err, stdout, stderr) {
            log.info('svn update done')
        })
        redisutil.retrieveGeneral(condition).then(function (itemList) {
            var result = []

            itemList.forEach(function (item) {
                if (item) {
                    var keysPart = item.key.split(':')
                    result.push({
                        "case_count": item.data.caseCount,
                        "desc": item.data.module_desc?item.data.module_desc:"请在用例中填写模块描述",
                        "project_name": keysPart[2],
                        "suite_name": keysPart[4],
                        "module": "",
                        "sub_module": keysPart[4],
                        "version": keysPart[3],
                        "red_mark":item.data.module_desc?0:1
                    })
                }
            })
            res.json({
                succes: true,
                count: result.length,
                result: _.sortBy(result, ['project_name', 'version', 'sub_module'])
            })
        }).catch(function (err) {
            res.json({
                succes: false,
        ...
        exec(SvnCmd, { cwd: '/var/stf/testproject2' }, function (err, stdout, stderr) {
            dbapi.loadTestSuite(project, suite, version, page, pageSize, type).then(function (cursor) {
                cursor.toArray(function (err, cases) {
                    if (err) {
                        res.json({ success: false, result: null })
                    }
...
router.post('/loadTestCase', function (req, res) {
    ...
    if (global.useredis) {
        redisutil.retrieveCaseDetail(type, project, version, sub_module).then(function (result) {
            var cases = []
            ...
            return {
                'cases': _.sortBy(cases, ['logic_id']),
                'desc': result.length > 0 ? result[0].out_desc : '',
                'package': result.length > 0 ? result[0].package : '',
                'projectName': project,
                'suite_name': sub_module
            }
...
router.post('/rerun', function (req, res) {
    ...
    arr.forEach(function(ele){
        dbapi.loadFailTestCaseIds(taskId, ele).then(function (taskInfo) {
            ...
            if (target_ids.length) {
                _.assign(task, {cases: target_ids}, {serials: [ele]})
...
router.get('/:taskid/:serial/report', function (req, res) {
    ...
    dbapi.loadActivityDetail(taskid).then(function (task) {
        if (task) {
            arr.forEach(function(item){
                ...
                var target = _.find(task.detail, function (detail) {
                    return detail.serial == item
                })
                var logPath = fileutil.dirname(target.logs.report)
                failPath = fileutil.dirname(logPath)+'/fail/'
                log.info('logPath:', logPath)
                // 判断日志文件是否存在
                if (fileutil.exists(logPath)) {
                    logPathList[target.markName] = logPath
                    log.debug('for循环内pathlist:',logPathList)
                }
            })
        }
        log.debug('for循环外pathlist:',logPathList)
        return logPathList
    }).then(function(data){
        log.info('failPath,',failPath)
        execSync('rm -rf '+failPath,function(error,stdout,stderr){
            log.info(stdout)
            log.info(error)
            log.info(stderr)
        })
        if (!fileutil.exists(failPath)){
            fileutil.mkdirs(failPath)
            log.info('创建 '+failPath)
        }
        var ifm = data
        return ifm
    }).then(function(data){
        log.info('logPathList:',data)

        for (var key in data){
            log.info('key:',key)
            fileutil.mkdirs(failPath+key)
            var sourcePath = data[key]
            log.info('sourcePath:',sourcePath)
            if (key.indexOf(' ')!= -1){
                var start = key.indexOf(' ')
                var newStr = '\\'
                key = key.slice(0, start) + newStr +  key.slice(start)
            }
            var destPath = failPath+key
            log.info('destPath:',destPath)
            execSync('cp -rf '+sourcePath+' '+destPath)
            //fileutil.copy(data[key],failPath+key)
            log.info('cp -rf '+sourcePath+' '+destPath)
            }

    }).then(function(){
        log.info('11failPath:',failPath)
        ...
        res.attachment(taskid + '.zip')
        archive.pipe(res)
        archive.directory(failPath + '/', taskid)
        archive.finalize()
    })
})
...
router.get('/luaversion', function (req, res) {
    dbapi.getLuaVersion().then(function (cursor) {
        return Promise.promisify(cursor.toArray, cursor)()
...
router.get('/loadTaskInfo/:taskid', function(req, res){
    var taskid = req.params.taskid
    ...
    dbapi.loadTaskByTaskid(taskid,key).then(function(cursor){
