              ...
              fs.stat(zipPath, function (err, stat) {
                ...
                setTimeout(function () {
                  ...
                  fs.stat(zipFile, function(err, stats){
                    if(err){
                      var zip = spawn('zip', ['-rm', zipFile, tmp])
                      zip.stdout.on('data', function(){})
                      zip.stderr.on('data', function(data){
                        log.info('compress files stderr:', data.toString())
                      })
                      zip.on('error', function(err){
                        log.info('compress files error:', err.toString())
                      })
