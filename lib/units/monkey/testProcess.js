/**
 * Created by YoungLiu on 2018/5/8.
 */
...
function test(options, taskid) {
    ...
    testutil.createadb(options, serial, log)
        ...
        .then(function () {
            ...
            testutil.saveLogcat(adb, serial, cptLogPath + logcatLogName)
                .then(function(logStr){
                    crashlogfilter.logcatfilter(logStr, function(result){
                        if (result && result.errorType) {
                            rpldata.result.errorType = result.errorType
                            rpldata.result.reason = result.reason
                            if (appLaunchTime = 0){
                                rpldata.appLaunchTime = 0
                                rpldata.status = 'fail'
            ...
            return adb.shell(serial, startPerformanceServiceCMD)
                ...
                .then(function (orientation) {
                    return new Promise(function (resolve, reject) {
                        ...
                        rpldata.logs.monkeyFile = monkeyFile
        ...
        .catch(function (err) {
            ...
            if (deviceDisconn.test(err) || deviceDisconn.test(err.stack)) {
                ...
                rpldata.result = {errorType: 'device disconnect', reason: 'device disconnect'}
            }
    ...
    function monkey() {
        ...
        function runMonkey(cb) {
            ...
            adb.shell(serial, command)
                ...
                .timeout(1000 * 60 * 60)
                .then(function (output) {
                    monkeyOutput = output.toString().trim()
                    fs.writeFileSync(cptLogPath + monkeyLogName, monkeyOutput)
                })
                .catch(function (err) {
                    log.error('monkey run failed. err:', err)
                    if(/TimeoutError/i.test(err.toString())){
                        testutil.killAndroidProceBypkg(adb, serial, 'com.android.commands.monkey', log)
                    }else{
                        rpldata.status = 'fail'
                        ...
                        rpldata.result.reason = err.toString()
                    }
                })
                .finally(function(){
                    ...
                    screenCap()
                            ...
                            return new Promise(function (resolve, reject) {
                                ...
                                var waitPullScreenCap = setInterval(function () {
                                    if(pullScreenCapFinish){
                                        ...
                                        adb.shell(serial, 'rm -f ' + cptScreencapOnphonePath + serial + '*.jpg')
                                            .catch(function (err) {
                        ...
                        .then(function () {
                            if (monkeyAbortedRegExp.test(monkeyOutput)) {
                                rpldata.status = 'fail'
                                if(rpldata.statusCode != 2){
                                    rpldata.statusCode = 3
                                }
                                ...
                                crashlogfilter.monkeylogfilter(monkeyOutput, function (result) {
                                    if (result && result.errorType) {
                                        rpldata.result.errorType = result.errorType
                                        rpldata.result.reason = result.reason
                                    }
                                })
                            } else if(!rpldata.result.errorType){
    ...
    function taskprc() {
        ...
        function clearUpPhone() {
            ...
            testutil.uninstallAPK(adb, serial, pkg, log)
                .catch(function (err) {
