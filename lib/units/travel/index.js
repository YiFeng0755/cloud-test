...
module.exports = function (req, taskid) {
  ...
  var taskListData = {
    ...
    allstatus: 'unfinish',
    detail: [],
    caseVersion: req.body.version?req.body.version:'V1',
    memo: req.body.memo,
  ...
  var type = '遍历测试'
  dbapi2.getCaseByProject(taskListData.projectName,taskListData.caseVersion,type).then(function(cursor){
    log.debug('projectName,caseVersion,type:',taskListData.projectName,taskListData.caseVersion,type)
    var casesIdList = []
    cursor.toArray(function(err,cases){
      log.debug('cases:',cases)

      cases.forEach(function(element){
        log.debug('id:',element.id)
        casesIdList.push(element.id)
      })
      taskListData.cases = casesIdList
    })
    return casesIdList
  }).then(function(cases) {
    var data = {}
    dbapi.loadApkinfo(id).then(function (apkinfo) {
      ...
      log.info('taskListData:', taskListData)
      log.info('taskListData.cases:',taskListData.cases)
      log.info('cases:',cases)
      dbapi.getPycharmPro(cases).then(function (cursor) {
