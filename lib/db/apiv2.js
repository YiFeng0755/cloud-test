/**
 * Created by YoungLiu on 2018/6/7.
 */
...
dbapi.loadTaskList = function (condition) {
    ...
    query = query.without('detail').map(function (task) {
        ...
        return {
            ...
            completedDevices: task('completedDevice').default([]).distinct(),
...
dbapi.deleteCase = function (condition) {
    ...
    //})
    var query = r.table('testCase').filter(condition).delete()
    return db.run(query)
}
...
dbapi.loadUITest = function (taskid, serial, modules, status) {
    ...
    var query = r.db('stf').table('tasklist').get(taskid).do(function (task) {
        return r.branch(
            task,
            r.branch(
                task('tasktype').eq('performTest'),
                task('detail').filter({ serial: serial }).map(function(taskdetail) {
                    return {
                        caseId: "",
                        caseDesc:  "",
                        endtime:  "",
                        ...
                        suiteDesc:  "",
                        error:  "",
                        screenCaps: taskdetail('performanceDetail')('screen').default([]).filter(function (c) {
                            return c.ne('')
                        }).map(function (c) {
                            return c.split('/var/stf/file').nth(1)
                        })
...
dbapi.getLuaVersion = function() {
    var query = r.db('stf').table('luaversion')('version').default('')
    return db.run(query)
}

dbapi.loadTaskByTaskid = function(taskid,key){
    if (key){
        log.info(typeof(key))
        var query = r.db('stf').table('tasklist').get(taskid)(key).default('')
    }
    else{
        var query = r.db('stf').table('tasklist').get(taskid)
    }
    return db.run(query)
}
