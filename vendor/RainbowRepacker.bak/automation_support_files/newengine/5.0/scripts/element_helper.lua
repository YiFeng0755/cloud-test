...
local function foreach_sub(w, fn)
    ...
    for _ , c in tableview_cell_pair(w.cells or {}) do
        fn(c)
    end
end
...
local function get_nodes_by_xpath(xpath)
    ...
    for _ , q in ipairs(queries) do
        select_nodes(Window.main_window, q, nodes)
    end
...
local function dump_widget_info(o, dump_children, xpath , scale)
    ...
    if r.type == 'TabelView' then
        r.cells = {}
        dump_to(r.cells, o.cells, tableview_cell_pair)
    end
...
local function get_element_by_text(w, text, elements)
    ...
    foreach_sub(w, function(c)
        if c.visible and c.text and c.text == text then
            table.insert(elements, c)
        end
...
function getElements(args)
    ...
    if #root_parents == 0 then root_parents[#root_parents+1] = Window.main_window end
...
function setElementText(args)
    local args = decode_json(args)
    if args == nil then
        -- return dict_set_string('Elements', 'setElementText', 'false')
        return nil;
     end
